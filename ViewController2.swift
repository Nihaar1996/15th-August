...
let tap: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(UIInputViewController.dismissKeyboard))
    ...
    func saveText() {
        ...
        defaults.setValue(NameofContact.text, forKey: "contactno")
    }
...
//        CLLocation.distanceFromLocation(CLLocation);.
    ...
    @IBAction func sendTextMessageButtonTapped(sender: UIButton) {
        ...
        if (messageComposer.canSendText()) {
            ...
            let city = prefs.stringForKey("contactno")
            // Obtain a configured MFMessageComposeViewController
            let messageComposeVC = messageComposer.configuredMessageComposeViewController()
            
            
            let textMessageRecipients = ["\(city)"] // for pre-populating the recipients list (optional, depending on your needs)
